...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if User.objects.filter(username="mike13").exists():
            self.stdout.write(
                self.style.SUCCESS(
                    "Database has already been populated. Cancelling the operation."
                )
            )
            return
        ...
        mike = User.objects.create_user(
            username="mike13", password="really_strong_password123"
        )
        mike.first_name = "Mike"
        mike.last_name = "Smith"
        mike.save()

        jess = User.objects.create_user(
            username="jess_", password="really_strong_password123"
        )
        jess.first_name = "Jess"
        jess.last_name = "Brown"
        jess.save()

        johnny = User.objects.create_user(
            username="johnny", password="really_strong_password123"
        )
        ...
        website_article.categories.add(
            programming, system_administration, seo_optimization
        )
